...
import LksListColumn from "@/Components/Column/LksListColumn";
...
const ListLKS = (props) => {
    ...
    const getData = (page, searchText) => {
        axios
            .get(
                config.base_url +
                    `/lks/simple?page=${page}&status=1,2,3,4,5,6&name=${searchText}`,
                {
                    auth: {
                        username: "sipelaku",
                ...
                <div className="bg-white w-full overflow-hidden h-full relative rounded-md shadow">
                    ...
                    <div className="min-h-content-table flex flex-col justify-between ">
                        <DataTable
                            ...
                            data={dataLks.data}
                            columns={LksListColumn.apply(null, [page])}
                            striped
